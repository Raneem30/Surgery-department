...
| 5 | **Doctor** | `ssn` | `department_code` | Department |
| 6 | **Treats** | `(patient_number, doctor_ssn)` | `patient_number` | Patient |
| | | | `doctor_ssn` | Doctor |
| 7 | **Prescription** | `prescription_id` | `doctor_ssn` | Doctor |
...
| | | | `doctor_ssn` | Doctor |
| | | | `room_id` | Room |
| | | | `case_id` | Surgery_Case |
| 12 | **Scan_Document** | `scan_id` | `patient_number` | Patient |
| | | | `doctor_ssn` | Doctor |
| 13 | **Procedure** | `procedure_code` | — | — |
| 14 | **Admission** | `admission_id` | `patient_number` | Patient |
| 15 | **Vital_Sign** | `vital_id` | `patient_number` | Patient |
| | | | `recorded_by_ssn` | Doctor |
| 16 | **Surgery_Case** | `case_id` | `patient_number` | Patient |
| | | | `procedure_code` | Procedure |
| | | | `admission_id` | Admission |
| 17 | **Surgery_Schedule** | `schedule_id` | `case_id` (UNIQUE) | Surgery_Case |
| | | | `or_room_id` | Room |
| 18 | **Surgical_Team_Assignment** | `(case_id, doctor_ssn, role)` | `case_id` | Surgery_Case |
| | | | `doctor_ssn` | Doctor |
| 19 | **IntraOp_Event** | `event_id` | `case_id` | Surgery_Case |
| 20 | **Specimen** | `specimen_id` | `case_id` | Surgery_Case |
| 21 | **Implant_Device** | `implant_id` | `case_id` | Surgery_Case |
| 22 | **PACU_Record** | `pacu_id` | `case_id` (UNIQUE) | Surgery_Case |
| 23 | **Surgical_Count** | `count_id` | `case_id` | Surgery_Case |
| | | | `verified_by_nurse_ssn` | Doctor |
...
| Treats | `patient_number` + `doctor_ssn` | M:N junction |
| Prescription_Medication | `prescription_id` + `medication_id` | M:N junction |
| Surgical_Team_Assignment | `case_id` + `doctor_ssn` + `role` | M:N with role discriminator |
...
| Payment | `appointment_id` | One payment per appointment (removed — legacy ref) |
...
| `idx_appointment_doctor` | Clinic_Appointment | `doctor_ssn` | FK lookup |
...
| `idx_treats_doctor` | Treats | `doctor_ssn` | FK lookup |
...
| `idx_team_doctor` | Surgical_Team_Assignment | `doctor_ssn` | FK lookup |
...
## Dropped Tables (removed from general clinic schema)

| Table | Reason |
|-------|--------|
| Geo_Location | Geolocation not needed for surgery dept |
| Contact_Inquiry | Customer support belongs in separate module |
| User | Authentication in separate security module |
| Payment | Billing in separate finance module |
